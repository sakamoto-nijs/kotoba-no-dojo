...
// 階層3：カテゴリー・レベルを選んだ後、さらに問題セット（1〜50のうち、実際に問題があるものだけ）を選ぶ画面。
// お気に入り（FAV）ではこの画面を通らず、レベル選択から直接モード画面へ進む。
function SetSelect({ modeKey, level, fullList, setNameMap, onSelect, onExit }) {
  const counts = {};
  fullList.filter((item) => item.level === level).forEach((item) => {
    const n = item.setNo || 1;
    counts[n] = (counts[n] || 0) + 1;
  });
  const setNos = Object.keys(counts).map(Number).sort((a, b) => a - b);

  return (
    <div className="max-w-xl mx-auto">
      <TopBar title={`${MODE_TITLES[modeKey]}（${level}）`} onExit={onExit} />
      <div style={{ fontSize: 13, color: COLORS.inkSoft, marginBottom: 18, fontFamily: KLEE, textAlign: "center" }}>
        取り組みたい問題セットを選んでください
      </div>
      {setNos.length === 0 ? (
        <div style={{ textAlign: "center", color: COLORS.inkFaint, fontFamily: KLEE, padding: "40px 0" }}>
          このレベルにはまだ問題がありません
        </div>
      ) : (
...
export default function App({
  ...
  // お気に入り（FAV）はセット横断の個人コレクションなので、セット選択画面を飛ばして直接モード画面へ進む
  const chooseLevel = (lv) => {
    setSelectedLevel(lv);
    setSelectedSetNo(null);
    setScreen(lv === "FAV" ? pendingMode : "set");
  };
  const chooseSet = (setNo) => { setSelectedSetNo(setNo); setScreen(pendingMode); };
  ...
  const modes = [
    { key: "flashcardReading", titleJa: "①フラッシュカード読(よ)み方(かた)", titleEn: "Flashcards (Reading)", desc: "単語を見て読み方を覚える", icon: BookOpen, disabled: flashcardReadingList.length === 0 },
    { key: "flashcardMeaning", titleJa: "②フラッシュカード意味(いみ)", titleEn: "Flashcards (Meaning)", desc: "単語を見て意味を覚える", icon: Languages, disabled: flashcardMeaningList.length === 0 },
    { key: "vocab4", titleJa: "③単語(たんご)4択(たく)問題(もんだい)", titleEn: "Word Quiz (4 Choices)", desc: "意味を見て単語を選ぶ", icon: ListChecks, disabled: vocab4List.length < 4 },
    ...
    { key: "grammar4", titleJa: "⑤文法(ぶんぽう)4択(たく)問題(もんだい)", titleEn: "Grammar Quiz (4 Choices)", desc: "括弧に入る言葉を4択で選ぶ", icon: PenLine, disabled: grammarList.length === 0 },
    { key: "kakitori", titleJa: "⑥漢字(かんじ)書(か)き取(と)り", titleEn: "Kanji Writing (Handwritten)", desc: "手書きで漢字を書いて答える", icon: PenTool, disabled: kakitoriList.length === 0 },
    { key: "vocab4choice", titleJa: "⑦語彙(ごい)4択(たく)問題(もんだい)", titleEn: "Vocabulary Quiz (4 Choices)", desc: "語彙の問題を4択で選ぶ", icon: BookOpenCheck, disabled: vocab4ChoiceList.length === 0 },
    { key: "kanji4choice", titleJa: "⑧漢字(かんじ)4択(たく)問題(もんだい)", titleEn: "Kanji Quiz (4 Choices)", desc: "漢字の問題を4択で選ぶ", icon: Hash, disabled: kanji4ChoiceList.length === 0 },
    { key: "reading", titleJa: "⑨読解(どっかい)問題(もんだい)", titleEn: "Reading Comprehension", desc: "文章を読んで設問に答える", icon: BookOpenText, disabled: readingList.length === 0 },
    { key: "reorder", titleJa: "⑩並(なら)べ替(か)え問題(もんだい)", titleEn: "Word Order (Reorder)", desc: "カードを正しい順番に並べ替える", icon: GripVertical, disabled: reorderList.length === 0 },
  ];
  ...
  // 階層3：お気に入り（FAV）以外は、レベルの中からさらに選んだ問題セット（set_no）で絞り込む
  const bySet = (list) => (selectedLevel === "FAV" ? list : list.filter((item) => (item.setNo || 1) === selectedSetNo));
  const activeFlashcardReading = bySet(levelFlashcardReading);
  const activeFlashcardMeaning = bySet(levelFlashcardMeaning);
  const activeVocab4 = bySet(levelVocab4);
  const activeKanji = bySet(levelKanji);
  const activeGrammar = bySet(levelGrammar);
  const activeKakitori = bySet(levelKakitori);
  const activeVocab4Choice = bySet(levelVocab4Choice);
  const activeKanji4Choice = bySet(levelKanji4Choice);
  const activeReading = bySet(levelReading);
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: 500, fontFamily: SANS, color: COLORS.ink }} className="w-full p-6 kotoba-dojo-root">
      ...
      {screen === "set" && pendingMode && selectedLevel && selectedLevel !== "FAV" && (
        <SetSelect
          modeKey={pendingMode}
          level={selectedLevel}
          fullList={modeDataMap[pendingMode].fullList}
      ...
      {screen === "reorder" && <ReorderMode list={activeReorder} level={selectedLevel} favSet={favReorder} onToggleFav={toggleFavReorder} onAnswer={reportAnswer} onExit={backFromMode} />}
