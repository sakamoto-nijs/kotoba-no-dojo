...
function parseSetNo(row, idx, problems) {
  const raw = (row.set_no || "").toString().trim();
  if (!raw) return 1;
  const n = parseInt(raw, 10);
  if (!n || n < 1 || n > SET_NO_MAX) {
    problems.push(`${idx + 2}行目: set_noは1〜${SET_NO_MAX}の数字にしてください → 1として扱いました`);
    return 1;
  }
  return n;
}
...
function parseCSVText(text) {
  ...
  result.data.forEach((row, idx) => {
    ...
    if (WORD_BASED_TYPES.includes(type) || type === "kakitori") {
      ...
      const parsed = parseBlankChoiceRow(row, type, level, setNo, idx, problems);
      ...
      rows.push({ type, level, set_no: setNo, blank: row.blank.trim(), cards });
    } else if (type) {
