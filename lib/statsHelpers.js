...
// questions/question_set_namesのDB上のtype列との対応表。
// ほとんどのモードはmodeKey＝typeで一致するが、⑤文法4択だけmodeKeyが"grammar4"
// （DBのtypeは"grammar"）とズレているため、type列を使ってルックアップする箇所
// （セット名の検索など）では、このマップを通してからキーを組み立てる。
export const MODE_KEY_TO_QUESTION_TYPE = {
  grammar4: "grammar",
};
...
export function buildStats({ progressRows, sessionRows, questionInfoMap }) {
  const byModeLevel = {};
  ALL_MODES.forEach((m) => {
    byModeLevel[m] = {};
  ...
  (progressRows || []).forEach((p) => {
    ...
    const info = questionInfoMap ? questionInfoMap.get(p.question_id) : null;
...
// 特定の学生・特定のモード・特定のレベルについて、問題セット（set_no）ごとの内訳を計算する。
// 教員ダッシュボードで「96%（45問）」のようなセルをクリックした時に、
// 「どのセクションをどう取り組んだか」を表示するために使う。
// 正答率はbuildStatsと同じ考え方（同じ問題は直近の解答結果を採用）、
// 学習回数はやり直しも含めた生の解答回数（attempts）。
export function buildSectionBreakdown({ progressRows, mode, level, questionInfoMap }) {
  const bySection = new Map(); // setNo -> { attempts, latest: Map<question_id, row> }

  (progressRows || []).forEach((p) => {
    if (p.mode !== mode) return;
    const info = questionInfoMap ? questionInfoMap.get(p.question_id) : null;
    if (!info || info.level !== level) return;
    const setNo = info.setNo || 1;

    if (!bySection.has(setNo)) bySection.set(setNo, { attempts: 0, latest: new Map() });
    const bucket = bySection.get(setNo);
    bucket.attempts += 1;

    const existing = bucket.latest.get(p.question_id);
    if (!existing || p.answered_at > existing.answered_at) {
      bucket.latest.set(p.question_id, p);
    }
  });

  const result = [];
  bySection.forEach((bucket, setNo) => {
    let total = 0, correct = 0;
    bucket.latest.forEach((p) => { total += 1; if (p.correct) correct += 1; });
    result.push({ setNo, total, correct, attempts: bucket.attempts });
  });
  result.sort((a, b) => a.setNo - b.setNo);
  return result;
}
