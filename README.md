...
- 教員管理画面（学生登録・進捗閲覧・CSVによる問題管理・学生の一括登録・パスワード再設定・学生/クラスの削除・問題セット名の設定）
...
- 学生画面のナビゲーションは「①カテゴリー→②レベル→③問題セット」の3階層。③問題セットはCSVの`set_no`列（1〜50、未指定なら1）で割り当てる。セットの名前（「1.第1課」のような表示名）はカテゴリー×レベルごとに教員管理画面の「問題セット」からつけられる（つけなければ番号のみ表示）。お気に入り（FAV）はセットをまたぐ個人コレクションのため、③のセット選択はスキップされる。学生画面には、実際に1件以上問題があるセットのみ表示される
